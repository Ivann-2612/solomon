...
describe('progression', () => {
  it('linear next room', () => {
    expect(nextRoom(1, { seals: 0, sealHere: false })).toBe(2);
  });
  it('bonus room after collecting constellation seal in 4th room', () => {
    expect(nextRoom(4, { seals: 0, sealHere: true })).toBe(101);
    expect(nextRoom(101, { seals: 0, sealHere: false })).toBe(5);
  });
  it('page of time after room 20 with 5+ seals', () => {
    expect(nextRoom(20, { seals: 5, sealHere: false })).toBe(201);
    expect(nextRoom(201, { seals: 5, sealHere: false })).toBe(21);
    expect(nextRoom(20, { seals: 4, sealHere: false })).toBe(21);
  });
  it('page of space after room 44 with 9+ seals', () => {
    expect(nextRoom(44, { seals: 9, sealHere: false })).toBe(202);
    expect(nextRoom(44, { seals: 8, sealHere: false })).toBe(45);
  });
  it('princess after 48 with 8+ seals', () => {
    expect(nextRoom(48, { seals: 8, sealHere: false })).toBe(203);
    expect(nextRoom(203, { seals: 8, sealHere: false })).toBe(49);
    expect(nextRoom(48, { seals: 7, sealHere: false })).toBe(49);
  });
